...
`docs/CONTRACTS.md` (backend `10.0.10.10`, mon `10.0.10.20`),
...
| Variable | Source |
|---|---|
| `GRAFANA_ADMIN_PASSWORD` | `8byte/jenkins` -> `admin_password` (one admin password for Jenkins and Grafana; generated by Terraform before hosts boot). |
...
password then lives in Grafana's own database). The password is generated by
Terraform, not set by `scripts/put-secrets.sh`; read it with
`aws secretsmanager get-secret-value --secret-id 8byte/jenkins --query SecretString --output text | jq -r .admin_password`.
