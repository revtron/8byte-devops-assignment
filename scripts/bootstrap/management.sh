...
# AL2023 mounts /tmp as a tmpfs of half the RAM (~950 MiB on a t3.small);
# Jenkins' disk-space monitor takes the built-in node offline below 1 GiB of
# free temp space and nothing ever builds. Keep temp files on the root volume.
JENKINS_TMP="$JENKINS_HOME/tmp"
install -d -m 0750 -o jenkins -g jenkins "$JENKINS_TMP"

# Jenkins listens on all interfaces; the security group admits :8080 only
# from admin_cidr (the same boundary as SSH). The public URL is taken from
# IMDS so JCasC can set the root URL to http://<EIP>:8080/ (links in
# notifications and the GitHub status point somewhere reachable).
IMDS_TOKEN="$(curl -sS -X PUT -H 'X-aws-ec2-metadata-token-ttl-seconds: 60' http://169.254.169.254/latest/api/token 2>/dev/null || true)"
PUBLIC_IP="$(curl -sS -H "X-aws-ec2-metadata-token: $IMDS_TOKEN" http://169.254.169.254/latest/meta-data/public-ipv4 2>/dev/null || true)"
JENKINS_PUBLIC_URL="http://${PUBLIC_IP:-localhost}:8080/"
log "Jenkins root URL: $JENKINS_PUBLIC_URL"
...
umask 077
{
  ...
  echo "[Service]"
  sd_env JAVA_OPTS "-Djava.awt.headless=true -Djenkins.install.runSetupWizard=false -Djava.io.tmpdir=$JENKINS_TMP"
  sd_env JENKINS_LISTEN_ADDRESS "0.0.0.0"
  sd_env JENKINS_PORT "8080"
  sd_env JENKINS_PUBLIC_URL "$JENKINS_PUBLIC_URL"
